...
# Push OrangeHRM image to ECR by tagging the local export produced by pull_orangehrm_image.sh.
# Optional environment variables:
#   AWS_PROFILE, AWS_ACCOUNT_ID, AWS_REGION, ECR_PREFIX, ECR_REPO_ORANGEHRM
...
ECR_REPO_ORANGEHRM="${ECR_REPO_ORANGEHRM:-orangehrm}"
...
ensure_repo() {
  ...
  if ! aws --profile "${AWS_PROFILE}" ecr describe-repositories --repository-names "${repo}" --region "${AWS_REGION}" >/dev/null 2>&1; then
    ...
    echo "[orangehrm] Created ECR repo: ${repo}"
  fi
}

ensure_local_image() {
  local img="${LOCAL_PREFIX}/orangehrm:latest"
  if ! docker image inspect "${img}" >/dev/null 2>&1; then
    echo "[orangehrm] Local image ${img} not found. Run scripts/pull_orangehrm_image.sh first."
    exit 1
  fi
}

main() {
  local repo="${ECR_PREFIX}/${ECR_REPO_ORANGEHRM}"
  local ecr_uri="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${repo}"
  local local_img="${LOCAL_PREFIX}/orangehrm:latest"

  login_ecr
  ensure_repo "${repo}"
  ensure_local_image

  echo "[orangehrm] Tagging ${local_img} as ${ecr_uri}:latest"
  docker tag "${local_img}" "${ecr_uri}:latest"
  docker push "${ecr_uri}:latest"
  echo "[orangehrm] Pushed ${ecr_uri}:latest"
}
