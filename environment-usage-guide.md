...
高校3年生でも迷わず動かせるように、やることをシンプルにまとめました。  
・招待メールを開いてパスワードを決める  
・コントロールサイトでサービスを起動する  
・60分使わないと自動で止まることを覚えておく  

---

## 1. まず最初に
1. Keycloak の招待メールを開く
2. 「メールアドレス確認」→ パスワード設定 → サインイン完了
3. あとは各サービスにアクセスすれば SSO で入れます（n8n だけ別手順）

### 起動/停止はここで
- コントロールサイト: `https://control.smic-aiops.jp/`
- ここで「起動」「停止」、スケジュール確認ができます。
- Keycloak / Zulip / n8n は平日 17:00–22:00、土日祝 08:00–23:00 に自動起動。それ以外は止まります。ほかのサービスは最初は止まっています。

---

## 2. サインインの仕組み
- **Keycloak SSO**: Sulu, Exastro ITA (Web/API), GitLab, phpMyAdmin (ALB OIDC), pgAdmin, GROWI, CMDBuild-r2u, Odoo, OrangeHRM などは、開くと Keycloak のサインイン画面に移動します。
...
- **それ以外**: ふだんは止まっています。必要なときにコントロールサイトで起動。
- **アイドル自動停止**: 60分リクエストが無いと止まります（自動起動時間中はスケジュール優先）。
...
- **招待メールが来ない**: SMTP 設定が必要。Keycloak / n8n / GitLab / Zulip / pgAdmin などのメール送信設定を確認。

---

## 6. 運用メモ
- 使う時間と自動停止がかぶらないようにスケジュールを調整してから作業する。
- Keycloak の realm は定期的にバックアップ（Terraform 変更前は特に）。
- シークレットを変えたら SSM を更新し、`scripts/redeploy_<service>.sh` で ECS を再デプロイ。
