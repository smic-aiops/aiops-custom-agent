...
# Push phpMyAdmin image to ECR by tagging the local export produced by pull_phpmyadmin_image.sh.
...
if [ -z "${ECR_REPO_PHPMYADMIN:-}" ]; then
  ECR_REPO_PHPMYADMIN="$(terraform output -raw ecr_repo_phpmyadmin 2>/dev/null || echo "phpmyadmin")"
fi
...
if [ -z "${PHPMYADMIN_IMAGE_TAG:-}" ]; then
  PHPMYADMIN_IMAGE_TAG="$(terraform output -raw phpmyadmin_image_tag 2>/dev/null || echo "latest")"
fi
...
ensure_repo() {
  ...
  if ! aws --profile "${AWS_PROFILE}" ecr describe-repositories --repository-names "${repo}" --region "${AWS_REGION}" >/dev/null 2>&1; then
    ...
    echo "[phpmyadmin] Created ECR repo: ${repo}"
  fi
}

ensure_local_image() {
  local img="${LOCAL_PREFIX}/phpmyadmin:latest"
  if ! docker image inspect "${img}" >/dev/null 2>&1; then
    echo "[phpmyadmin] Local image ${img} not found. Run scripts/pull_phpmyadmin_image.sh first."
    exit 1
  fi
}

main() {
  local repo="${ECR_PREFIX}/${ECR_REPO_PHPMYADMIN}"
  local ecr_uri="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${repo}"
  local local_img="${LOCAL_PREFIX}/phpmyadmin:latest"

  login_ecr
  ensure_repo "${repo}"
  ensure_local_image

  echo "[phpmyadmin] Tagging ${local_img} as ${ecr_uri}:latest"
  docker tag "${local_img}" "${ecr_uri}:latest"
  docker push "${ecr_uri}:latest"
  echo "[phpmyadmin] Pushed ${ecr_uri}:latest"
}
