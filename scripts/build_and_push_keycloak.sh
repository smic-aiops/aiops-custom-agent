...
if [ -z "${ECR_REPO_KEYCLOAK:-}" ]; then
  ECR_REPO_KEYCLOAK="$(terraform output -raw ecr_repo_keycloak 2>/dev/null || echo "keycloak")"
fi
...
if [ -z "${KEYCLOAK_IMAGE_TAG:-}" ]; then
  KEYCLOAK_IMAGE_TAG="$(terraform output -raw keycloak_image_tag 2>/dev/null || echo "26.4.7")"
fi
...
ensure_local_image() {
  ...
  if ! docker image inspect "${img}" >/dev/null 2>&1; then
    echo "[keycloak] Local image ${img} not found. Run scripts/pull_keycloak_image.sh first."
    exit 1
  fi
}
